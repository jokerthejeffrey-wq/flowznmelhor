...
<div class="login-only">
    <div class="login-shell">
        ...
        {% if auth_mode == "register" %}
            ...
            <div class="login-sub">Use a real email domain. Temporary or non-mail domains are blocked.</div>
...
<div class="app">
    <div class="side">
        ...
        <div class="user-mini" onclick="showAccount(document.getElementById('menuAccount'))" style="cursor:pointer;">
            <div class="pfp-box" id="sidePfpBox">
                {% if pfp_url %}
...
function updateSidePfp(){
    const box = document.getElementById("sidePfpBox");

    if(!box) return;

    if(pfpUrl){
        box.innerHTML = `<img src="${escapeAttr(pfpUrl)}" alt="pfp">`;
    }else{
        box.textContent = username.charAt(0).toUpperCase();
    }
}
...
async function checkForUpdates(){
    ...
    try{
        ...
        if(data.db_updated_at !== dbUpdatedAt){
            ...
            if(data.pfp_url){
                pfpUrl = data.pfp_url + "&cache=" + Date.now();
            }
    ...
    try{
        ...
        if(content){
            content.innerHTML = `
                ...
                <div class="small">Something broke while loading the page. Check the browser console.</div>
                <br>
